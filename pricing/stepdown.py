...
class StepDownPayoff:
	...
	def initialize_timeGrid(self, timeGrid: mx.TimeDateGrid):
		if not isinstance(timeGrid, mx.TimeDateGrid):
			raise Exception('timeDateGrid is required')
...
def build_stepdown():
	notional = 10000
	issue_date = mx.Date(22, 8, 2012)
	maturity_date = mx.Date(22, 8, 2015)
	...
	coupons = [(mx.Date(13, 2, 2013), 0.9, 0.06),
			   (mx.Date(13, 8, 2013), 0.9, 0.12),
			   (mx.Date(13, 2, 2014), 0.85, 0.18),
			   (mx.Date(13, 8, 2014), 0.85, 0.24),
			   (mx.Date(13, 2, 2015), 0.8, 0.30),
			   (mx.Date(13, 8, 2015), 0.8, 0.36)]
...
def build_scenario(overwrite=True):
	...
	timeGrid = mx.TimeDateGrid_Equal(refDate, 3, 365)

	# random 
	rsg = xen.Rsg(sampleNum=10000)
...
class McPricer:
	...
	def npv(scen, payoff: StepDownPayoff, discount_curve):
...
def test():
	...
	stepdown = build_stepdown()
	curve = ts.FlatForward(refDate, 0.03, mx.Actual365Fixed(), mx.Semiannual)
	npv = McPricer().npv(stepdown, curve)
	print(npv)
